...
This creates a hash chain similar to git. While we cannot cryptographically *prove* a node came from a specific model (that would require API providers to sign responses), we can:
- Detect any tampering with the chain after creation
- Store raw API responses as evidence of origin
- Provide an audit trail sufficient for research credibility

> **Note**: True cryptographic proof of model identity is a limitation of current APIs, not our system. We design for the possibility of future API provider support for signed responses.
